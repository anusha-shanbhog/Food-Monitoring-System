...
static const char vernum[] = "1.0.0";
String devstr =  "esp32cam";
int max_frames = 300;
...
void handleNewMessages(int numNewMessages) {
  ...
  for (int i = 0; i < numNewMessages; i++) {
    ...
    if (text == "/start") {
      ...
      welcome += "/flashphoto: photo with flash LED on\n";
...
void the_camera_loop (void* pvParameter) {
  ...
  if (avi_enabled) {
    ...
    digitalWrite(33, HIGH);          // built-in red LED light off
    avi_end_time = millis();
...
void do_eprom_read() {
  ...
  if (ed.eprom_good == MagicNumber) {
    ...
    */
    ...
    chat_id = "912172204";
    BOTtoken = "6144004733:AAESpU5PFtfQZFEYTq3i9zyhJRZWvo__gA4";
    timezone = "UTC+5:30";
    devstr = "deskpir";
...
void send_the_picture() {
  digitalWrite(33, LOW);          // built-in red LED light on
  currentByte = 0;
